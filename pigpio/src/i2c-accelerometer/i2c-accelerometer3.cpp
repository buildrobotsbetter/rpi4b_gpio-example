...
bool readBytes(int, unsigned char, char*);
...
int main()
{
    ...
    while (isRunning)
    {
        ...
        if (readBytes(i2cHandle, 0x32, data))
        {
            short dataX_1 = (data[1] << 8) | data[0];
            short dataY_1 = (data[3] << 8) | data[2];
            short dataZ_1 = (data[5] << 8) | data[4];
        ...
        {
            std::cout << fmt::format("Error reading data") << std::endl;
        }
...
bool readBytes(int i2cHandle, unsigned char regAddr, char* data)
{
    int result = i2cReadI2CBlockData(i2cHandle, regAddr, data, 6);
    if (result < 0)
    {
        std::cout << fmt::format("Error with i2cReadI2CBlockData({:x}) (Error: {})", regAddr, result) << std::endl;            
        return false;
    }
...
{
    int result = i2cWriteByteData(i2cHandle, regAddr, value);
    if (result != 0)
    {
        std::cout << fmt::format("Error with i2cWriteByteData({:x}) (Error: {})", regAddr, result) << std::endl;               
        return false;
    }
